...
class Wav2Vec2Model(BaseFairseqModel):
    ...
    def __init__(self, args):
        ...
        self.feature_extractor = ConvFeatureExtractionModel(
            ...
            layer_norm_num=1 if args.encoder_layers==12 else 7,
        )
    ...
    def load_pretrained_model(self, args):
        pretraining_path = args.load_pretrained_model_from

        if pretraining_path:
            model_state = torch.load(pretraining_path)
            self.load_state_dict(model_state['model'], strict=False)

            logger.info(f"loaded pretrained model from: {pretraining_path}")
        ...
        for i, cl in enumerate(conv_layers):
            ...
            self.conv_layers.append(
                block(
                    ...
                    k,
                    stride,
                    is_layer_norm=mode == "layer_norm" and i < layer_norm_num,
