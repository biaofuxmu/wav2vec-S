...
class FairseqSimulSTAgent(SpeechAgent):
    ...
    def load_model_vocab(self, args):
        ...
        if args.user_dir is not None:
            ...
            print("new user_dir: ", args.user_dir)
        ...
        sys.stderr.write(
            "new w2v2_model_path: " + str(state["cfg"]["model"].__dict__["w2v2_model_path"]) + "\n")
        sys.stderr.write("new user_dir: " + str(state["cfg"]["task"].__dict__["user_dir"]) + "\n")
    ...
    def policy(self, states):
        ...
        if states.incremental_states["steps"]["src"] < self.prev_src:   # 处理cif缩短后长度反而比tgt_indices短的问题
            ...
            if not states.finish_read():
                ...
                # self.update_states_write(states)
                # return WRITE_ACTION
                return None
