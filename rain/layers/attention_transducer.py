...
class IsolatedDecoder(TransformerDecoder):
    ...
    def extract_features_scriptable(
        ...
        # embed tokens and positions B x T x C
...
class MHAJointNet(nn.Module):
    def __init__(
        ...
        self.step_mode = getattr(args,'step_mode', "constant")
        self.init_step = self.downsample
        if self.downsample == 32:
            self.scale = 8
        else:
            self.scale = 16

    def sampling_decision_step(self):
        if self.step_mode == "constant":
            return
        if self.training:
            if self.step_mode == "random":
                ...
                self.downsample = steps[idx] * self.scale

    def _gen_group_mask(self, encoder_out, encoder_padding_mask):
        B,T = encoder_padding_mask.shape
        self.sampling_decision_step()
        with torch.no_grad():
            group_num = math.ceil(T /self.downsample)
            group_pos= torch.arange(1,group_num + 1)*self.downsample
            tidx= torch.arange(T)
