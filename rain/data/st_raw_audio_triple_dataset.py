...
def get_features_from_npy_or_audio(path):
    ...
    return np.load(path) if ext == ".npy" else get_waveform(path)[0]
...
class SpeechToTextDataset(FairseqDataset):
    ...
    def collater(self, samples: List[Tuple[int, torch.Tensor, torch.Tensor]]) -> Dict:
        if len(samples) == 0:
            return {}
        idx_list, frame_list, translation_list, transcription_list = [], [], [], []
        frame_lens, translation_lens, transcription_lens = [], [], []
        for idx, frame, translation, transcription in samples:
            assert translation is not None, transcription is not None
            idx_list.append(idx)
            frame_list.append(frame)
            frame_lens.append(frame.size(0))
            
            translation_list.append(translation)
            if len(translation):
                translation_lens.append(len(translation))
            else:
                translation_lens.append(1)
            
            transcription_list.append(transcription)
            if len(transcription):
                transcription_lens.append(len(transcription))
            else:
                transcription_lens.append(1)

        indices = torch.tensor(idx_list, dtype=torch.long)
        frames = _collate_frames(frame_list, self.data_cfg.use_audio_input)
        # sort samples by descending number of frames
        n_frames = torch.tensor(frame_lens, dtype=torch.long)
        ...
        transcription_lengths = torch.tensor(
            transcription_lens, dtype=torch.long
        ).index_select(0, order)
        prev_source = fairseq_data_utils.collate_tokens(
            transcription_list,
            self.tgt_dict.pad(),
            self.tgt_dict.eos(),
            left_pad=False,
        ...
        prev_source = prev_source.index_select(0, order)
        ...
        target_lengths = torch.tensor(
            translation_lens, dtype=torch.long
        ).index_select(0, order)
        prev_target = fairseq_data_utils.collate_tokens(
            translation_list,
            self.tgt_dict.pad(),
            self.tgt_dict.eos(),
            left_pad=False,
        ...
        prev_target = prev_target.index_select(0, order)

        ntokens_src = sum(transcription_lengths).item()
        ntokens_tgt = sum(target_lengths).item()
