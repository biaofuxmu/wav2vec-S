...
import logging
import copy
logger = logging.getLogger('transducer.searcher')
...
class OnlineFeatureExtractor:
    ...
    def __init__(self, args):
        self.shift_size = SHIFT_SIZE # args.shift_size
        self.window_size = WINDOW_SIZE # args.window_size
        assert self.window_size >= self.shift_size

        self.sample_rate = SAMPLE_RATE # args.sample_rate
...
class FullyTransducerAgent(Agent):
    def __init__(self, args):
        ...
        self.main_context = getattr(args, "main_context", None)

        if args.w2v2_model_path is None:
            raise ValueError("--w2v2-model-path needed")

        arg_overrides = {"w2v2_model_path": self.w2v2_model_path}

        if self.main_context is not None:
            arg_overrides["main_context"] = self.main_context

        state = checkpoint_utils.load_checkpoint_to_cpu(args.model_path)

        task_cfg = state["cfg"]["task"]
        task_cfg.data = args.data_bin
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--task-type", default="mt", metavar='type',
        ...
        parser.add_argument(
            "--data-bin", default="exp_data/must_filtered2", type=str,
        ...
        parser.add_argument(
            "--w2v2-model-path", default=None, type=str,
    ...
    def policy(self, states):
        ...
        source = states.source.value
        source_length = len(source)

        if source_length == 0:
            return READ_ACTION

        current_frame = source_length // 320
        # current_frame = self.searcher.models.models[0].encoder.w2v2_model.feature_extractor(source.unsqueeze(0).cuda()).size(2)

        if states.finish_read():
            self.infer(states)
        if (self.processed_frames == 0 and current_frame >= self.expected_init_frames()) or \
            (self.processed_frames > 0 and current_frame - self.processed_frames >= self.expected_step_frames()):

            self.infer(states)
            self.processed_frames = current_frame
    ...
    def infer(self, states):
        ...
        src_indices = states.source.value.unsqueeze(0).cuda()
        src_lengths = torch.LongTensor([states.source.value.size(0)]).cuda()

        max_steps = int(self.max_len_a * (src_indices.shape[1] // 160) - self.max_len_b - self.prev_tokens.shape[1])
